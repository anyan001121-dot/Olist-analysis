...
| 2 | 这个效应经因果推断验证为**真实且几乎无混杂偏差** | PSM 估计 ATT **+44.74pp**（95% CI [43.37, 46.10]）；103 个协变量匹配后 SMD 全部 < 0.045 |
...
真实数据最大的价值不在"更真实"，而在于它会**主动骗你**。本项目在写任何分析前先做了数据质量探查，之后又补做了一轮自查（`python/00_audit.py`），一共抓到四个会直接导致结论错误的陷阱。
...
修法：视图层新增 `is_late_int` / `is_bad_int` / `is_top_int`，对不适用的行置 NULL，交给 `AVG()` 自动跳过；全库聚合改用新字段，不再对布尔字段直接写 `CASE WHEN`。这个修复顺带统一了此前月度延迟率的两套口径（一处过滤未送达订单、一处没过滤，峰值月相差 0.49pp），也让 PSM 的卖家历史协变量改成了不掺未来订单的扩展窗口计算（ATT 从 +45.05pp 修正为 +44.74pp，结论方向不变）。完整的自查过程见 `python/00_audit.py`。
...
> **一个与预期相反的结果，值得单独说明。**
>
> 事前假设是"朴素对比会高估延迟的伤害"，但匹配后发现**选择偏差只有 0.04pp（0.1%）**——朴素估计本来就近乎无偏。
>
...
| 模型 | 特征 | AUC | KS | 用途 |
|---|---|---|---|---|
| A | 含履约结果（delivery_days、is_late…） | 0.731 | 0.356 | 事后归因 |
| B | **仅下单时可得特征** | 0.637 | 0.193 | 线上预警 |

![预警模型](output/figures/09_warning_lift.png)

模型 B 的 AUC 0.637 不高，但它是唯一能上线的那个。按它的分数排序：

- **TOP 10%**（2,112 单）实际差评率 **25.0%**，是基线的 **2.47 倍**，覆盖 24.8% 的差评
...
**3. 用预警模型做主动干预**
对下单时预警分数 TOP 10% 的订单（差评率 25.0%，2.47 倍基线）做提前催发货、主动告知物流进度。具体触达比例取决于单次干预成本与一条差评的代价。

**4. 收益测算（分档，不画大饼）**

| 情形 | 延迟率目标 | 可消除差评 | 占差评总量 |
|---|---|---|---|
| A | 8.0% → 5.0%（对齐 SP 州现有水平） | ~1,183 条 | 10.9% |
| B | 8.0% → 6.5%（治理最差的线路与卖家） | ~616 条 | 5.7% |

理论上界（延迟率归零）是 3,072 条、占 28.3%，但那是不可达成的，不应写进 OKR。
