...
**Olist 电商平台履约质量与客户体验分析** ｜ 独立项目 ｜ SQL / Python / 因果推断

- 基于 Olist 巴西电商公开数据集（9 张表、9.8 万订单、11.2 万明细、10 万条评论），用 DuckDB 搭建 DWD 视图层统一指标口径，识别并修正三处数据陷阱：`customer_id` 为订单级键导致复购率误算为 0、同日拆单使复购率虚高 29%、多表一对多 JOIN 使 GMV 虚高 4.57%
- 定位平台差评核心成因为配送延迟：延迟订单差评率 54.0% vs 按时 9.2%，并通过剂量反应分析发现伤害呈断崖式（延迟 1-3 天差评率即从 11.7% 跳至 32.2%），据此提出"提升时效确定性优先于压缩平均时效"
- 使用倾向得分匹配（110 维协变量，匹配后 SMD 全部 < 0.045）估计因果效应 ATT +45.05pp，并用 IPW 与回归调整交叉验证（四法极差 0.67pp）；发现选择偏差仅 0.3%，据此推翻"偏远地区差评不可避免"的假设
- 将延迟责任拆解至运输段（85.5%）与卖家备货段（14.5%），结合卖家分层数据（四档延迟率 7.3%~8.1% 无差异）论证问题在物流网络而非卖家治理
- 构建下单时差评预警模型（LightGBM，TOP 10% 提升 2.43 倍）；排查并修复时间穿越泄漏（卖家历史特征由全期均值改为扩展窗口），使 AUC 从虚高的 0.932 回落至真实的 0.636，归因结论由此反转
